...
import io.bootique.di.spi.OptionalBinderAdapter;
...
public class OptionalBinder<T> {
    ...
    private final OptionalBinderAdapter<T> delegate;

    private OptionalBinder(OptionalBinderAdapter<T> delegate) {
        this.delegate = delegate;
    }

    public static <T> OptionalBinder<T> newOptionalBinder(Binder binder, Class<T> type) {
        return new OptionalBinder<>(newOptionalAdapter(binder, Key.get(type)));
    }
    ...
    public static <T> OptionalBinder<T> newOptionalBinder(Binder binder, Key<T> type) {
        return new OptionalBinder<>(newOptionalAdapter(binder, type));
    }

    private static <T> OptionalBinderAdapter<T> newOptionalAdapter(Binder binder, Key<T> key) {
        return new OptionalBinderAdapter<>(binder, key);
    }
